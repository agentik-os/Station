...
def _prepare_workspace(definition: dict) -> Path:
    ...
    (target / ".agentik-agent.json").write_text(
        json.dumps({"id": definition["id"], "version": definition["version"],
                    "profile": definition.get("profile"), "os": definition.get("os") or []}, indent=2) + "\n",
        encoding="utf-8",
    )
    return target


def _profile_command(definition: dict, workspace: Path) -> list[str] | None:
    profile = str(definition.get("profile") or "").strip()
    if not profile:
        return None
    if not _ID.fullmatch(profile):
        raise RuntimeError("agent profile id is invalid")
    profile_home = Path.home() / ".hermes" / "profiles" / profile
    if not profile_home.is_dir():
        raise RuntimeError(f"agent profile is not installed: {profile}")
    ...
    if launcher_value:
        ...
        if home not in launcher.parents or not launcher.is_file() or not os.access(launcher, os.X_OK):
            raise RuntimeError("agent profile launcher is not an executable inside the current Linux home")
        return [str(launcher), "--in", str(workspace)]
    return [shutil.which("hermes") or "hermes", "-p", profile, "--in", str(workspace)]


def _bind_profile_runtime(session: str, definition: dict, workspace: Path) -> None:
    command = _profile_command(definition, workspace)
    if not command:
        return
    row = _runtime_row(session)
    if not row:
        raise RuntimeError("agent runtime registration is missing")
    try:
        current = json.loads(row.get("command_json") or "[]")
    except (TypeError, ValueError):
        current = []
    if current == command:
        return
    pane = _pane(session)
    result = _run(["rmux", "respawn-pane", "-k", "-t", pane, "-c", str(workspace), *command])
    if result.returncode:
        raise RuntimeError("could not bind agent runtime to its Hermes profile")
    path = Path.home() / ".agentik/runtime.db"
    db = sqlite3.connect(path)
    try:
        db.execute("UPDATE runtime_sessions SET command_json=? WHERE name=?", (json.dumps(command), session))
        db.commit()
    finally:
        db.close()
    ...
    try:
        if action == "list":
            return tool_result({"success": True, "agents": [
                {key: item.get(key) for key in ("id", "name", "version", "description", "scope", "profile", "os")}
        ...
        if action == "start":
            ...
            _bind_profile_runtime(session, definition, workspace)
            if instruction:
                _send(session, instruction)
            return tool_result({"success": True, "agent": agent_id, "session": session,
                                "created": row is None, "profile": definition.get("profile"),
