...
def _plan_key(session_id:str,turn_id:str)->tuple[str,str,str]|None:
 session=str(session_id or "").strip(); turn=str(turn_id or "").strip()
 if not session or not turn: return None
 try: home=str(get_hermes_home().resolve())
 except Exception: return None  # noqa: BLE001 - policy gate must fail closed
 return home,session,turn


def require_plan_before_work(tool_name:str="",args:dict|None=None,session_id:str="",turn_id:str="",**_kwargs):
 """Block operational tools until the current session turn has applied a todo plan."""
 name=str(tool_name or "").strip()
 if name=="todo" or name in _PLAN_EXEMPT_TOOLS:
  return None
 key=_plan_key(session_id,turn_id)
 if key is None:
  return {"action":"block","message":"Plan Mode required: missing session/turn identity; apply a canonical todo plan before operational work."}
...
def record_applied_plan(tool_name:str="",args:dict|None=None,session_id:str="",turn_id:str="",result:Any=None,status:str="",**_kwargs):
