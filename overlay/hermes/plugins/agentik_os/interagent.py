...
def interagent_prompt(_session_info: dict | None = None) -> str:
    return (
        ...
        "inline target-bot mention; the target acknowledges and works only in that thread and must never call station_interagent back "
        "for an acknowledgement or status echo, and never create a self-thread for its own work. Do not claim "
