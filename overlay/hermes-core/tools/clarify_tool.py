...
def _invoke_callback(callback, question, choices, multi_select, surface=None):
    """Invoke a callback once, forwarding only supported optional keywords."""
    import inspect

    accepts_multi = False
    accepts_surface = False
    try:
        params = inspect.signature(callback).parameters
        has_var_kwargs = any(
            p.kind == inspect.Parameter.VAR_KEYWORD for p in params.values()
        )
        accepts_multi = "multi_select" in params or has_var_kwargs
        accepts_surface = "surface" in params or has_var_kwargs
    ...
    if accepts_surface and surface:
...
def _build_decision_surface(
    *, question: str, title: str = "", state: str = "", context: str = "",
    ...
    return {
        "decision_id": "decision-" + hashlib.sha256(digest_source).hexdigest()[:12],
        "title": str(title or "").strip() or "Decision required",
        "state": str(state or "").strip() or "Awaiting your decision",
        "context": str(context or "").strip(),
        "established": _clean_text_list(established),
        "target": clean_target,
        "consequences": _clean_text_list(consequences),
...
CLARIFY_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "title": {"type": "string", "description": "Short specific title for the decision surface."},
            "state": {"type": "string", "description": "Current status in one short line."},
            "context": {"type": "string", "description": "Why this decision is needed now."},
            "established": {
                "type": "array", "items": {"type": "string"},
                "description": "Facts already established; do not ask for them again.",
            },
            "target": {"type": "string", "description": "Exact object, service, file, channel, or scope affected."},
            "consequences": {
                "type": "array", "items": {"type": "string"}, "maxItems": MAX_CHOICES,
                "description": "One material consequence per choice, in the same order as choices.",
            },
...
registry.register(
    ...
    handler=lambda args, **kw: clarify_tool(
        ...
        established=args.get("established"),
