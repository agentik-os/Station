...
class AIAgent:
    ...
    def _touch_activity(
        ...
        *,
        ...
        if progress:
            ...
            self._last_progress_desc = bounded_desc
    ...
    def get_activity_summary(self) -> dict:
        ...
        progress_ts = getattr(self, "_last_progress_ts", None)
        now = time.time()
        snapshot.update(
            {
                "last_progress_at": progress_ts,
                "last_progress_ts": progress_ts,
                "last_progress_description": getattr(
                    self, "_last_progress_desc", ""
                ),
                "last_progress_desc": getattr(self, "_last_progress_desc", ""),
                "seconds_since_progress": max(0.0, now - progress_ts)
                if progress_ts is not None
                else snapshot.get("seconds_since_activity", 0.0),
            }
        )
        return snapshot
