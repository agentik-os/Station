...
snapshot_service=$source_root/systemd/agk-fleet-snapshot.service
snapshot_timer=$source_root/systemd/agk-fleet-snapshot.timer
snapshot_script=$source_root/scripts/fleet_snapshot.py
kanban_setup=$source_root/scripts/setup-fleet-kanban.sh
core_os_installer=$source_root/scripts/install_core_os_packages.py
core_os_source=$source_root/os-packages
...
test -f "$snapshot_service" && test -f "$snapshot_timer" && test -f "$snapshot_script" && test -f "$kanban_setup" && test -f "$core_os_installer" && test -d "$core_os_source" || {
  echo "AGK Fleet snapshot collector assets are missing" >&2
  exit 1
}
...
install -m 0755 "$snapshot_script" "$install_root/scripts/fleet_snapshot.py"
install -m 0755 "$kanban_setup" "$install_root/scripts/setup-fleet-kanban.sh"
"$install_root/scripts/setup-fleet-kanban.sh"
install -m 0755 "$core_os_installer" "$install_root/scripts/install_core_os_packages.py"
rm -rf "$install_root/os-packages"
cp -a "$core_os_source" "$install_root/os-packages"
"$install_root/venv/bin/python" "$install_root/scripts/install_core_os_packages.py" \
  --source "$install_root/os-packages" --registry /opt/agentik/os-registry --assign-fleet
install -m 0644 "$snapshot_service" /etc/systemd/system/agk-fleet-snapshot.service
install -m 0644 "$snapshot_timer" /etc/systemd/system/agk-fleet-snapshot.timer
systemctl daemon-reload
systemctl enable --now agk-fleet-snapshot.timer
systemctl start agk-fleet-snapshot.service
test -r /var/lib/agk-terminal/fleet/fleet-snapshot.json || {
  echo "AGK Fleet snapshot was not generated" >&2
  exit 1
}
