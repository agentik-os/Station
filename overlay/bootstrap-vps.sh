...
if [ "$skip_packages" = false ]; then
  ...
  run env DEBIAN_FRONTEND=noninteractive apt-get install -y \
    ...
    python3 python3-pip python3-venv sudo unzip libssl-dev ffmpeg
fi
...
phase "Install the pinned Caveman local proxy CLI"
if [ "$dry_run" = true ]; then
  echo "  + npm install -g --ignore-scripts @caveman-ai/cli@1.2.5"
else
  node_major=$(node --version | tr -d v | cut -d. -f1)
  [ "$node_major" -ge 22 ] || {
    echo "Caveman requires Node.js 22.13 or newer; found $(node --version)" >&2
    exit 1
  }
  npm install -g --ignore-scripts '@caveman-ai/cli@1.2.5'
...
phase "Initialize one isolated Kanban station per AGK boundary"
if [ "$dry_run" = true ]; then
  echo "  + operator/agentik/mission/private station boards"
else
