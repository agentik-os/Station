...
install -m 0755 "$repo_root/scripts/build-agency-skill.py" \
...
install -m 0755 "$repo_root/scripts/fleet_snapshot.py" \
  "$install_root/scripts/fleet_snapshot.py"
install -m 0755 "$repo_root/scripts/setup-fleet-kanban.sh" \
  "$install_root/scripts/setup-fleet-kanban.sh"
install -m 0755 "$repo_root/scripts/rotate_discord_token.py" \
...
install -d -m 0755 "$install_root/hermes-core/gateway"
...
if [ "$system_install" = true ]; then
  ...
  install -m 0644 "$repo_root/systemd/agk-fleet-snapshot.service" \
...
if [ "$system_install" = true ]; then
  ...
  # Named Hermes profiles remain inside their owning Linux boundary but need
  # the same pinned plugins, skills, rules, and voice runtime configuration.
  for profile_user in operator agentik mission private; do
    profile_home=$(getent passwd "$profile_user" | cut -d: -f6)
    [ -d "$profile_home/.hermes/profiles" ] || continue
    while IFS= read -r -d '' named_home; do
      [ -f "$named_home/config.yaml" ] || continue
      sudo -u "$profile_user" env \
        HOME="$profile_home" \
        ...
        PATH="$profile_home/.local/bin:$bin_dir:/usr/local/bin:/usr/bin:/bin" \
        "$install_root/scripts/sync-hermes.sh"
    done < <(find "$profile_home/.hermes/profiles" -mindepth 1 -maxdepth 1 \
      -type d -print0)
  done
...
echo "Operator installs automatically configure the lightweight Discord account monitor (OpenAI + Claude Code)."
echo "Monitor root/category: 1542505218569150585 · OpenAI channel: 1542505478679171164 · interval: usage_monitor_interval_seconds=300"
if [ -x "$install_root/scripts/setup-obsidian-livesync.sh" ]; then
  "$install_root/scripts/setup-obsidian-livesync.sh"
fi
