...
def test_plan_mode_allows_read_only_discovery_before_plan_but_not_mutation():
 module=load(); identity={"session_id":"session-discovery","turn_id":"turn-discovery"}
 for tool_name in ("tool_search","tool_describe","read_file","search_files","web_search","web_extract"):
  assert module.require_plan_before_work(tool_name=tool_name,args={},**identity) is None
 for tool_name in ("terminal","write_file","patch","station_interagent","agk_completion"):
  blocked=module.require_plan_before_work(tool_name=tool_name,args={},**identity)
...
def test_plan_authorization_survives_cron_tool_cycle_turn_id_drift():
 module=load()
 plan='{"todos":[{"id":"x","content":"Work","status":"in_progress"}]}'
 module.record_applied_plan(
  tool_name="todo",args={"todos":[{"id":"x","content":"Work","status":"in_progress"}]},
  session_id="cron-session",task_id="cron-task",turn_id="cron-turn-1",status="ok",result=plan,
 )
 assert module.require_plan_before_work(
  tool_name="terminal",session_id="cron-session",task_id="cron-task",turn_id="cron-turn-2"
 ) is None
 blocked=module.require_plan_before_work(
  tool_name="terminal",session_id="cron-session",task_id="next-cron-task",turn_id="cron-turn-3"
 )
 assert blocked["action"]=="block"
...
def test_add_requirement_resolves_the_sole_mission_prompt_when_omitted(tmp_path,monkeypatch):
 module=load()
 monkeypatch.setenv("HERMES_HOME",str(tmp_path/".hermes"))
 monkeypatch.setenv("AGK_TERMINAL_ROOT",str(Path(__file__).resolve().parents[1]/"overlay"))
 archived=json.loads(asyncio.run(module.handle_completion({
  "action":"archive","text":"Implement the complete request","source":"test",
  "session_id":"session-autonomy","profile":"operator",
 })))
 created=json.loads(asyncio.run(module.handle_completion({
  "action":"create_mission","prompt_id":archived["prompt_id"],
 })))
 result=json.loads(asyncio.run(module.handle_completion({
  "action":"add_requirement","mission_id":created["mission_id"],
  "text":"Continue safe in-scope work autonomously",
 })))
 assert result["success"] is True
 store=module.open_store()
 try:
  row=store.get_requirement(result["requirement_id"])
  assert row["prompt_id"]==archived["prompt_id"]
 finally: store.close()


def test_completion_prompt_continues_newly_discovered_in_scope_work_without_approval():
 prompt=load().completion_prompt()
 assert "newly discovered work required by the current request" in prompt
 assert "A0-A3" in prompt
 assert "unrelated backlog" in prompt
